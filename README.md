...
- 🖱️ Mouse-wheel scrolling and click selection
- 📥 Excel export through a Next.js route handler
- 🔁 Retry logic with exponential backoff
- ⏱️ Timeout handling for IMF requests
- 🛡️ Structured backend and frontend error handling
- ☁️ Vercel-friendly serverless architecture
...
- 🏷️ Sets `Content-Disposition: attachment; filename="IMF_Data.xlsx"`
...
- 🔁 3 retry attempts
- ⏱️ 10 second timeout per IMF request
...
## 🧹 Important Build/Dev Note

This project now isolates Next.js output directories to prevent stale chunk errors:
...
- ⚠️ Some country/indicator combinations may still return no data from IMF, which is handled as a clean app response rather than a crash.
- ⚠️ `xlsx` currently reports an upstream `npm audit` advisory with no published fix available at install time.
...
- ✅ Themed modern UI
- ✅ Isolated Next.js build/dev output handling
